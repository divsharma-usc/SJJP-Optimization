...
plotly.tools.set_credentials_file(username='tutrinhkt94', api_key='vSrhCEpX6ADg7esoVCbc') 
import copy                            
...
def initialization():
    ...
    Job1Nodes = [Node0,Node1,Node2,Node3]   
...
class Ant:
    def __init__(self, num):
        ...
        self.tabu = []
        self.position = -1
        self.T = [[0 for i in range(numNodes)] for j in range(numNodes)] 
        self.pheromoneAccumulator = [[0 for i in range(numNodes)] for j in range(numNodes)] 
        self.transitionRuleMatrix = [[0 for i in range(numNodes)] for j in range(numNodes)] 
        self.makespan = 0
        self.species = 'none'
...
def parameterInitialization(numAnts, numCycles):
    ...
    K = numAnts 
    C = numCycles 
    Q1 = float(20) atePheromoneAccumulation()
    Q2 = float(5) 
